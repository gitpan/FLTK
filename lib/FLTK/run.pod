...
=for hackers xs/run.xs line 447
...
=item C<my $okay = FLTK::add_fd( $fh, $events, \&callback, $args );>X<my_okay_FLTK_add_fd_fh_events_callback_args_>
...
=for hackers xs/run.xs line 560
...
=for hackers xs/run.xs line 591
...
=for hackers xs/run.xs line 477
...
=item C<my $okay = FLTK::remove_fd( $fh, $when );>X<my_okay_FLTK_remove_fd_fh_when_>
...
=for hackers xs/run.xs line 655

=item C<my $okay = FLTK::remove_fd( $fileno, $when );>X<my_okay_FLTK_remove_fd_fileno_when_>

Removes the handle (if it exists) with the particular fileno.

=pod 

=for hackers xs/run.xs line 681
...
=for hackers xs/run.xs line 496
...
=item C<FLTK::repeat_timeout( $time, \&callback, $args );>X<FLTK_repeat_timeout_time_callback_args_>
...
=for git $Id: run.xs 6090ead 2011-01-30 18:51:54Z sanko@cpan.org $
